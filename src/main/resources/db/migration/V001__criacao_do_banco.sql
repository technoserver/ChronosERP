...
-- ------------------------------------------------------------
-- Para cada campo desta tabela deve ser criada uma função na biblioteca de restrições da aplicação.
--
-- Outros campos podem ser criados de acordo com a necessidade.
-- ------------------------------------------------------------

CREATE TABLE RESTRICAO_SISTEMA (
  ID         SERIAL NOT NULL,
  TAXA_MAIOR DECIMAL(18, 6),
  TAXA_MENOR DECIMAL(18, 16),
  DATA_MAIOR INTEGER,
  DATA_MENOR INTEGER,
  PRIMARY KEY (ID)
);
...
CREATE INDEX FK_PAIS_INDICE_ECONOMICO
  ON INDICE_ECONOMICO (ID_PAIS);
...
CREATE INDEX FK_SUB_GRUPO_GRUPO
  ON PRODUTO_SUBGRUPO (ID_GRUPO);
...
CREATE INDEX FK_PESSOA_TELEFONE
  ON PESSOA_TELEFONE (ID_PESSOA);
...
CREATE INDEX FK_PESSOA_JURIDICA
  ON PESSOA_JURIDICA (ID_PESSOA);
...
CREATE INDEX FK_PESSOA_ENDERECO
...
CREATE INDEX FK_PESSOA_CONTATO
  ON PESSOA_CONTATO (ID_PESSOA);
...
CREATE INDEX FK_BANCO_AGENCIA
  ON AGENCIA_BANCO (ID_BANCO);
...
CREATE INDEX FK_SIND_PATRO_CONTR
  ON CONTRIB_SIND_PATRONAL_CAB (ID_SINDICATO);
...
CREATE INDEX FK_PESSOA_TRANSPORTADORA
  ON TRANSPORTADORA (ID_PESSOA);
...
CREATE INDEX FK_CONTADOR_EMPRESA
  ON EMPRESA (ID_CONTADOR);
CREATE INDEX FK_SINDICATO_EMPRESA
  ON EMPRESA (ID_SINDICATO_PATRONAL);
CREATE INDEX FK_AUTO_REL_EMPRESA
...
CREATE INDEX FK_PESSOA_DAV_CAB
  ON DAV_CABECALHO (ID_PESSOA);
CREATE INDEX FK_EMPRESA_DAV_CAB
  ON DAV_CABECALHO (ID_EMPRESA);
...
CREATE INDEX FK_AGENCIA_CONTA
  ON CONTA_CAIXA (ID_AGENCIA_BANCO);
CREATE INDEX FK_EMPRESA_CONTA_CAIXA
  ON CONTA_CAIXA (ID_EMPRESA);
...
CREATE INDEX FK_EMPRESA_TIPO_RCTO
  ON FIN_TIPO_RECEBIMENTO (ID_EMPRESA);
CREATE INDEX FK_CONTA_CAIXA_TIPO_REC
  ON FIN_TIPO_RECEBIMENTO (ID_CONTA_CAIXA);
...
CREATE INDEX FK_PESSOA_PV_CAB
  ON PRE_VENDA_CABECALHO (ID_PESSOA);
CREATE INDEX FK_EMPRESA_PV_CAB
  ON PRE_VENDA_CABECALHO (ID_EMPRESA);
...
CREATE INDEX FK_ESTADO_CIVIL_PF
  ON PESSOA_FISICA (ID_ESTADO_CIVIL);
CREATE INDEX FK_PESSOA_FISICA
...
CREATE INDEX FK_PAPEL_FUNCAO
  ON PAPEL_FUNCAO (ID_PAPEL);
CREATE INDEX FK_FUNCAO_PAPEL
  ON PAPEL_FUNCAO (ID_FUNCAO);
...
CREATE INDEX FK_EMPRESA_TIPO_NF
  ON NOTA_FISCAL_TIPO (ID_EMPRESA);
CREATE INDEX FK_NF_TIPO_MODELO
...
CREATE INDEX FK_SITUACAO_FORNECEDOR
  ON FORNECEDOR (ID_SITUACAO_FOR_CLI);
CREATE INDEX FK_ATIVIDADE_FORNECEDOR
  ON FORNECEDOR (ID_ATIVIDADE_FOR_CLI);
...
CREATE INDEX FK_EMPRESA_TIPO_PGTO
  ON FIN_TIPO_PAGAMENTO (ID_EMPRESA);
...
CREATE INDEX FK_EMPRESA_STATUS_PARCELA
  ON FIN_STATUS_PARCELA (ID_EMPRESA);
...
CREATE INDEX FK_EMPRESA_NFE_INUT
  ON NFE_NUMERO_INUTILIZADO (ID_EMPRESA);
...
CREATE INDEX FK_EMPRESA_NFE_NUMERO
  ON NFE_NUMERO (ID_EMPRESA);
...
CREATE INDEX FK_EMPRESA_QUADRO_SOCIO
  ON QUADRO_SOCIETARIO (ID_EMPRESA);
...
CREATE INDEX FK_EMP_PL_CT_RESULTADO
  ON PLANO_CENTRO_RESULTADO (ID_EMPRESA);
...
CREATE INDEX FK_EMPRESA_PL_NT_FIN
  ON PLANO_NATUREZA_FINANCEIRA (ID_EMPRESA);
...
CREATE INDEX FK_CONTA_OP_CARTAO
  ON OPERADORA_CARTAO (ID_CONTA_CAIXA);
...
CREATE INDEX FK_QUADRO_SOCIO
  ON SOCIO (ID_QUADRO_SOCIETARIO);
...
CREATE INDEX FK_EMPRESA_CTE_CABECALHO
  ON CTE_CABECALHO (ID_EMPRESA);
...
CREATE INDEX FK_CTE_CAB_COMPONENTE
  ON CTE_COMPONENTE (ID_CTE_CABECALHO);
...
CREATE INDEX FK_CTE_CAB_DESTINATARIO
  ON CTE_DESTINATARIO (ID_CTE_CABECALHO);
...
CREATE INDEX FK_EMPRESA_CTE_CONFIG
  ON CTE_CONFIGURACAO (ID_EMPRESA);
...
CREATE INDEX FK_CTE_CAB_EXPEDIDOR
  ON CTE_EXPEDIDOR (ID_CTE_CABECALHO);
...
CREATE INDEX FK_CTE_CAB_INFORMACAO_NF
  ON CTE_INFORMACAO_NF_OUTROS (ID_CTE_CABECALHO);
...
CREATE INDEX FK_CTE_CAB_COLETA
  ON CTE_LOCAL_COLETA (ID_CTE_CABECALHO);
...
CREATE INDEX FK_CTE_CAB_FERROVIARIO
  ON CTE_FERROVIARIO (ID_CTE_CABECALHO);
...
CREATE INDEX FK_CTE_CAB_FATURA
  ON CTE_FATURA (ID_CTE_CABECALHO);
...
CREATE INDEX FK_CTE_FERRO_VIA
  ON CTE_FERROVIARIO_FERROVIA (ID_CTE_FERROVIARIO);
...
CREATE INDEX FK_CTE_INF_NF_CARGA
  ON CTE_INFORMACAO_NF_CARGA (ID_CTE_INFORMACAO_NF);
...
CREATE INDEX FK_CTE_FERRO_VAGAO
  ON CTE_FERROVIARIO_VAGAO (ID_CTE_FERROVIARIO);
...
CREATE INDEX FK_PLANO_CENTRO
  ON CENTRO_RESULTADO (ID_PLANO_CENTRO_RESULTADO);
...
CREATE INDEX FK_EMPRESA_COMISSAO_PERFIL
  ON COMISSAO_PERFIL (ID_EMPRESA);
...
CREATE INDEX FK_EMPRESA_PARAMETRO
  ON ADM_PARAMETRO (ID_EMPRESA);
...
CREATE INDEX FK_EMPRESA_ALMOXARIFADO
  ON ALMOXARIFADO (ID_EMPRESA);
...
CREATE INDEX FK_EMPRESA_CONVENIO
  ON CONVENIO (ID_EMPRESA);
...
CREATE INDEX FK_CONTR_SIND_PATRONAL
  ON CONTRIB_SIND_PATRONAL_DET (ID_CONTRIB_SIND_PATRONAL_CAB);
...
CREATE INDEX FK_CTE_CAB_AEREO
  ON CTE_AEREO (ID_CTE_CABECALHO);
...
CREATE INDEX FK_CTE_CAB_AQUAVIARIO
  ON CTE_AQUAVIARIO (ID_CTE_CABECALHO);
...
CREATE TABLE TRIBUT_OPERACAO_FISCAL (
  ...
  ESTOQUE_VERIFICADO        BOOLEAN DEFAULT FALSE,
  CONTABILIDADE             BOOLEAN DEFAULT FALSE,
  OBRIGACAO_FISCAL          BOOLEAN DEFAULT FALSE,
  DESTACA_IPI               BOOLEAN DEFAULT FALSE,
  DESTACA_PIS_COFINS        BOOLEAN DEFAULT FALSE,
  CALCULO_INSS              BOOLEAN DEFAULT FALSE,
  CALCULO_ISSQN             BOOLEAN DEFAULT FALSE,
  CALCULO_PIS_COFINS_RETIDO BOOLEAN DEFAULT FALSE,
  CALCULO_IRRF              BOOLEAN DEFAULT FALSE,
  OBSERVACAO                TEXT,
  PRIMARY KEY (ID),
  FOREIGN KEY (ID_EMPRESA)
  REFERENCES EMPRESA (ID)
...
CREATE INDEX FK_EMPRESA_OPERACAO_FISCAL
  ON TRIBUT_OPERACAO_FISCAL (ID_EMPRESA);
...
CREATE INDEX FK_EMPRESA_GRUPO_TRIBUTARIO
  ON TRIBUT_GRUPO_TRIBUTARIO (ID_EMPRESA);
...
CREATE INDEX FK_EMPRESA_ENDERECO
  ON EMPRESA_ENDERECO (ID_EMPRESA);
...
CREATE INDEX FK_EMRESA_TELEFONE
  ON EMPRESA_TELEFONE (ID_EMPRESA);
...
CREATE INDEX FK_EMPRESA_CONTATO
  ON EMPRESA_CONTATO (ID_EMPRESA);
...
CREATE INDEX FK_SOCIO_PART_SOCIETARIA
  ON SOCIO_PARTICIPACAO_SOCIETARIA (ID_SOCIO);
...
CREATE INDEX FK_EMPRESA_ICMS_CUSTOM
  ON TRIBUT_ICMS_CUSTOM_CAB (ID_EMPRESA);
...
CREATE INDEX FK_ICMS_CUSTOM_CAB_DET
  ON TRIBUT_ICMS_CUSTOM_DET (ID_TRIBUT_ICMS_CUSTOM_CAB);
...
CREATE INDEX FK_CONTA_CX_EXTRATO
  ON FIN_EXTRATO_CONTA_BANCO (ID_CONTA_CAIXA);
...
CREATE INDEX FK_CONTA_CX_FECHAMENTO
  ON FIN_FECHAMENTO_CAIXA_BANCO (ID_CONTA_CAIXA);
...
CREATE INDEX FK_CTE_CAB_RODOVIARIO
  ON CTE_RODOVIARIO (ID_CTE_CABECALHO);
...
CREATE INDEX FK_CTE_CAB_REMETENTE
  ON CTE_REMETENTE (ID_CTE_CABECALHO);
...
CREATE INDEX FK_CTE_ROD_MOTORISTA
  ON CTE_RODOVIARIO_MOTORISTA (ID_CTE_RODOVIARIO);
...
CREATE INDEX FK_CTE_ROD_LACRE
  ON CTE_RODOVIARIO_LACRE (ID_CTE_RODOVIARIO);
...
CREATE INDEX FK_CTE_CAB_RECEBEDOR
  ON CTE_RECEBEDOR (ID_CTE_CABECALHO);
...
CREATE INDEX FK_CTE_CAB_MULTI
  ON CTE_MULTIMODAL (ID_CTE_CABECALHO);
...
CREATE INDEX FK_CTE_CAB_ENTREGA
  ON CTE_LOCAL_ENTREGA (ID_CTE_CABECALHO);
...
CREATE INDEX FK_CTE_CAB_PERIGO
  ON CTE_PERIGOSO (ID_CTE_CABECALHO);
...
CREATE INDEX FK_CTE_CAB_PASSAGEM
  ON CTE_PASSAGEM (ID_CTE_CABECALHO);
...
CREATE INDEX FK_CTE_ROD_OCC
  ON CTE_RODOVIARIO_OCC (ID_CTE_RODOVIARIO);
...
CREATE INDEX FK_CTE_CAB_TOMADOR
  ON CTE_TOMADOR (ID_CTE_CABECALHO);
...
CREATE INDEX FK_CTE_CAB_SEGURO
  ON CTE_SEGURO (ID_CTE_CABECALHO);
...
CREATE INDEX FK_CTE_CAB_VEICULO_NOVO
  ON CTE_VEICULO_NOVO (ID_CTE_CABECALHO);
...
CREATE TABLE TRIBUT_ISS (
  ...
  indicador_exigibilidade    INTEGER,
...
CREATE INDEX FK_TRIBUT_OP_FISCAL_ISS
  ON TRIBUT_ISS (ID_TRIBUT_OPERACAO_FISCAL);
...
CREATE INDEX FK_CTE_ROD_PEDAGIO
  ON CTE_RODOVIARIO_PEDAGIO (ID_CTE_RODOVIARIO);
...
CREATE INDEX FK_CTE_ROD_VEICULO
  ON CTE_RODOVIARIO_VEICULO (ID_CTE_RODOVIARIO);
...
CREATE INDEX FK_RELACIONA_SOCIO_DEP
  ON SOCIO_DEPENDENTE (ID_TIPO_RELACIONAMENTO);
CREATE INDEX FK_SOCIO_DEPENDENTE
  ON SOCIO_DEPENDENTE (ID_SOCIO);
...
CREATE INDEX FK_EMPRESA_TALONARIO_CHEQUE
  ON TALONARIO_CHEQUE (ID_EMPRESA);
CREATE INDEX FK_CONTA_TALONARIO
  ON TALONARIO_CHEQUE (ID_CONTA_CAIXA);
...
CREATE INDEX FK_TRANSPORTADORA_MUNICIPIO
  ON TRANSPORTADORA_MUNICIPIO (ID_TRANSPORTADORA);
CREATE INDEX FK_MUNICIPIO_TRANSPORTADORA
  ON TRANSPORTADORA_MUNICIPIO (ID_MUNICIPIO);
...
CREATE INDEX FK_OP_FISCAL_CONFIGURA
  ON TRIBUT_CONFIGURA_OF_GT (ID_TRIBUT_OPERACAO_FISCAL);
CREATE INDEX FK_GRUPO_TRIB_CONFIGURA
  ON TRIBUT_CONFIGURA_OF_GT (ID_TRIBUT_GRUPO_TRIBUTARIO);
...
CREATE INDEX FK_IPI_RECEITA_DIPI
  ON TRIBUT_IPI_DIPI (ID_TIPO_RECEITA_DIPI);
CREATE INDEX FK_TRIBUT_OPERACAO_FISCAL_IPI
  ON TRIBUT_IPI_DIPI (ID_TRIBUT_OPERACAO_FISCAL);
...
CREATE INDEX FK_EMPRESA_COND_PGTO
  ON VENDA_CONDICOES_PAGAMENTO (ID_EMPRESA);
CREATE INDEX FK_TIPO_REC_COND_PAG
  ON VENDA_CONDICOES_PAGAMENTO (ID_FIN_TIPO_RECEBIMENTO);
...
CREATE INDEX FK_EMPRESA_PESSOA
  ON EMPRESA_PESSOA (ID_EMPRESA);
CREATE INDEX FK_PESSOA_EMPRESA
  ON EMPRESA_PESSOA (ID_PESSOA);
...
CREATE INDEX FK_EMPRESA_CNAE
  ON EMPRESA_CNAE (ID_EMPRESA);
CREATE INDEX FK_CNAE_EMPRESA
  ON EMPRESA_CNAE (ID_CNAE);
...
CREATE INDEX FK_DOC_ORIG_LCTO_PAGAR
  ON FIN_LANCAMENTO_PAGAR (ID_FIN_DOCUMENTO_ORIGEM);
CREATE INDEX FK_FORNECEDOR_LCTO_PAGAR
  ON FIN_LANCAMENTO_PAGAR (ID_FORNECEDOR);
...
CREATE INDEX FK_EMPRESA_CONF_BOLETO
  ON FIN_CONFIGURACAO_BOLETO (ID_EMPRESA);
CREATE INDEX FK_CTA_CX_CONF_BOLETO
  ON FIN_CONFIGURACAO_BOLETO (ID_CONTA_CAIXA);
...
CREATE INDEX FK_PESSOA_EMIT_CHEQUE
  ON FIN_CHEQUE_RECEBIDO (ID_PESSOA);
CREATE INDEX FK_CONTA_CAIXA_CHEQUE_REC
  ON FIN_CHEQUE_RECEBIDO (ID_CONTA_CAIXA);
...
CREATE INDEX FK_COTACAO_FORNECEDOR
  ON COMPRA_FORNECEDOR_COTACAO (ID_COMPRA_COTACAO);
CREATE INDEX FK_FORNECEDOR_COTACAO
  ON COMPRA_FORNECEDOR_COTACAO (ID_FORNECEDOR);
...
CREATE INDEX FK_TIPO_PEDIDO_COMPRA
  ON COMPRA_PEDIDO (ID_COMPRA_TIPO_PEDIDO);
CREATE INDEX FK_FORNECEDOR_PEDIDO_COMPRA
  ON COMPRA_PEDIDO (ID_FORNECEDOR);
...
CREATE INDEX FK_DOC_ORIG_PAGA_FIXO
  ON FIN_PAGAMENTO_FIXO (ID_FIN_DOCUMENTO_ORIGEM);
CREATE INDEX FK_FORNECEDOR_PAGA_FIXO
  ON FIN_PAGAMENTO_FIXO (ID_FORNECEDOR);
...
CREATE INDEX FK_STATUS_PARCELA_PAGAR
  ON FIN_PARCELA_PAGAR (ID_FIN_STATUS_PARCELA);
CREATE INDEX FK_LANCAMENTO_PARCELA
  ON FIN_PARCELA_PAGAR (ID_FIN_LANCAMENTO_PAGAR);
CREATE INDEX FK_CTA_CX_PAR_PAG
  ON FIN_PARCELA_PAGAR (ID_CONTA_CAIXA);
...
CREATE INDEX FK_PRODUTO_UNIDADE
  ON PRODUTO (ID_UNIDADE_PRODUTO);
CREATE INDEX FK_PRODUTO_SUBGRUPO
  ON PRODUTO (ID_SUBGRUPO);
CREATE INDEX FK_MARCA_PRODUTO
  ON PRODUTO (ID_MARCA_PRODUTO);
CREATE INDEX FK_GRUPO_TRIB_PRODUTO
  ON PRODUTO (ID_GRUPO_TRIBUTARIO);
CREATE INDEX FK_ALMOX_PRODUTO
  ON PRODUTO (ID_ALMOXARIFADO);
CREATE INDEX FK_ICMS_CUSTOM_PROD
  ON PRODUTO (ID_TRIBUT_ICMS_CUSTOM_CAB);
...
CREATE INDEX FK_SITUACAO_CLIENTE
  ON CLIENTE (ID_SITUACAO_FOR_CLI);
CREATE INDEX FK_ATIVIDADE_CLIENTE
  ON CLIENTE (ID_ATIVIDADE_FOR_CLI);
CREATE INDEX FK_PESSOA_CLIENTE
  ON CLIENTE (ID_PESSOA);
CREATE INDEX FK_OP_FISCAL_CLIENTE
  ON CLIENTE (ID_OPERACAO_FISCAL);
CREATE INDEX FK_CONVENIO_CLIENTE
  ON CLIENTE (ID_CONVENIO);
CREATE INDEX FK_TABELA_PRECO_CLIENTE
  ON CLIENTE (ID_TABELA_PRECO);
...
CREATE INDEX FK_COLABORADOR_SETOR
  ON COLABORADOR (ID_SETOR);
CREATE INDEX FK_CARGO_COLABORADOR
  ON COLABORADOR (ID_CARGO);
CREATE INDEX FK_TIPO_COLABORADOR
  ON COLABORADOR (ID_TIPO_COLABORADOR);
CREATE INDEX FK_NIVEL_FORMACAO_COLABORADOR
  ON COLABORADOR (ID_NIVEL_FORMACAO);
CREATE INDEX FK_PESSOA_COLABORADOR
  ON COLABORADOR (ID_PESSOA);
CREATE INDEX FK_SITUACAO_COLABORADOR
  ON COLABORADOR (ID_SITUACAO_COLABORADOR);
CREATE INDEX FK_TIPO_ADMISSAO_COLAB
  ON COLABORADOR (ID_TIPO_ADMISSAO);
CREATE INDEX FK_SINDICATO_COLAB
  ON COLABORADOR (ID_SINDICATO);

-- ------------------------------------------------------------
-- Tabela criada para controlar as solicitações remotas enviadas pelos PDVs.
-- ------------------------------------------------------------

CREATE TABLE PDV_SOLICITACAO_REMOTA (
  ID               SERIAL  NOT NULL,
  ID_COLABORADOR   INTEGER NOT NULL,
  DATA_SOLICITACAO DATE,
  HORA_SOLICITACAO VARCHAR(8),
  NOME_CAIXA       VARCHAR(100),
  NOME_OPERADOR    VARCHAR(100),
  DADOS_DOCUMENTO  TEXT,
  TIPO_SOLICITACAO CHAR(1),
  DEFERIDO         CHAR(1),
  DATA_LIBERACAO   DATE,
  HORA_LIBERACAO   VARCHAR(8),
  PRIMARY KEY (ID),
...
CREATE INDEX FK_COLABORADOR_SOLICITACA_PDV
  ON PDV_SOLICITACAO_REMOTA (ID_COLABORADOR);
CREATE INDEX IFK_FK_COLABORADOR_SOLICITACA_
  ON PDV_SOLICITACAO_REMOTA (ID_COLABORADOR);
...
CREATE INDEX FK_PLANO_NATUREZA_FINAN
  ON NATUREZA_FINANCEIRA (ID_PLANO_NATUREZA_FINANCEIRA);
...
CREATE INDEX FK_PRODUTO_CODIGO_ADD
  ON PRODUTO_CODIGO_ADICIONAL (ID_PRODUTO);
...
CREATE INDEX FK_PRODUTO_ALTERACAO_ITEM
  ON PRODUTO_ALTERACAO_ITEM (ID_PRODUTO);
...
CREATE INDEX FK_PRODUTO_PROMOCAO
  ON PRODUTO_PROMOCAO (ID_PRODUTO);
...
CREATE INDEX FK_COLAB_REQ_INTERNA
  ON REQUISICAO_INTERNA_CABECALHO (ID_COLABORADOR);
...
CREATE INDEX FK_CTE_CAB_DOC_ANTERIOR
  ON CTE_DOCUMENTO_ANTERIOR (ID_CTE_CABECALHO);
...
CREATE INDEX FK_CTE_CAB_DUPLICATA
  ON CTE_DUPLICATA (ID_CTE_CABECALHO);
...
CREATE INDEX FK_CTE_CAB_EMITENTE
  ON CTE_EMITENTE (ID_CTE_CABECALHO);
...
CREATE INDEX FK_CTE_CAB_DUTO
  ON CTE_DUTOVIARIO (ID_CTE_CABECALHO);
...
CREATE INDEX FK_CTE_CAB_CARGA
  ON CTE_CARGA (ID_CTE_CABECALHO);
...
CREATE INDEX FK_CTE_INF_NF_TRANSP
  ON CTE_INFORMACAO_NF_TRANSPORTE (ID_CTE_INFORMACAO_NF);
...
CREATE INDEX FK_CTE_INF_CARGA_LACRE
  ON CTE_INF_NF_CARGA_LACRE (ID_CTE_INFORMACAO_NF_CARGA);
...
CREATE INDEX FK_CTE_INF_NF_TRANS_LACRE
  ON CTE_INF_NF_TRANSPORTE_LACRE (ID_CTE_INFORMACAO_NF_TRANSP);
...
CREATE INDEX FK_TALONARIO_CHEQUE
  ON CHEQUE (ID_TALONARIO_CHEQUE);
...
CREATE INDEX FK_COLABORADOR_ROMANEIO
  ON VENDA_ROMANEIO_ENTREGA (ID_COLABORADOR);
...
CREATE INDEX FK_CONDICOES_PARCELAS
  ON VENDA_CONDICOES_PARCELAS (ID_VENDA_CONDICOES_PAGAMENTO);
...
CREATE INDEX FK_CTE_AQUA_BALSA
  ON CTE_AQUAVIARIO_BALSA (ID_CTE_AQUAVIARIO);
...
CREATE INDEX FK_TRIBUT_OPERACAO_FISCAL_PIS
  ON TRIBUT_PIS_COD_APURACAO (ID_TRIBUT_OPERACAO_FISCAL);
...
CREATE INDEX FK_TRIBUT_OPERACAO_FISCAL_COFINS
  ON TRIBUT_COFINS_COD_APURACAO (ID_TRIBUT_OPERACAO_FISCAL);
...
CREATE INDEX FK_CHEQUE_EMITIDO
  ON FIN_CHEQUE_EMITIDO (ID_CHEQUE);
...
CREATE INDEX FK_CLIENTE_COBRANCA
  ON FIN_COBRANCA (ID_CLIENTE);
...
CREATE INDEX FK_COBRANCA_PARCELA
  ON FIN_COBRANCA_PARCELA_RECEBER (ID_FIN_COBRANCA);
...
CREATE INDEX FK_PRODUTO_FICHA_TECNICA
  ON FICHA_TECNICA (ID_PRODUTO);
...
CREATE INDEX FK_COLAB_HIST_REAJUSTE
  ON ESTOQUE_REAJUSTE_CABECALHO (ID_COLABORADOR);
...
CREATE INDEX FK_TRIBUT_OPERACAO_FISCAL_ICMS
  ON TRIBUT_ICMS_UF (ID_TRIBUT_OPERACAO_FISCAL);
CREATE INDEX FK_TRIBUT_GRUPO_TRIBUTARIO_ICMS
  ON TRIBUT_ICMS_UF (ID_TRIBUT_GRUPO_TRIBUTARIO);
...
CREATE INDEX FK_LACRE_PRODUTO_ENTRADA
  ON PRODUTO_LACRE_ENTRADA (ID_LACRE_PRODUTO);
CREATE INDEX FK_PRODUTO_LACRE_ENTRADA
  ON PRODUTO_LACRE_ENTRADA (ID_PRODUTO);
...
CREATE INDEX FK_TABELA_PRECO
  ON TABELA_PRECO_PRODUTO (ID_TABELA_PRECO);
CREATE INDEX FK_PRODUTO_PRECO
  ON TABELA_PRECO_PRODUTO (ID_PRODUTO);
...
CREATE INDEX FK_UNIDADE_CONVERSAO
  ON UNIDADE_CONVERSAO (ID_UNIDADE_PRODUTO);
CREATE INDEX FK_PRODUTO_CONVERSAO_UND
  ON UNIDADE_CONVERSAO (ID_PRODUTO);
...
CREATE INDEX FK_REQ_INTERNA_CAB_DET
  ON REQUISICAO_INTERNA_DETALHE (ID_REQ_INTERNA_CABECALHO);
CREATE INDEX FK_PRODUTO_REQ_INTERNA
  ON REQUISICAO_INTERNA_DETALHE (ID_PRODUTO);
...
CREATE INDEX FK_COLABORADOR_USUARIO
  ON USUARIO (ID_COLABORADOR);
CREATE INDEX FK_PAPEL_USUARIO
  ON USUARIO (ID_PAPEL);
...
CREATE INDEX FK_PROD_COMBO_ITEM
  ON PRODUTO_COMBO_ITEM (ID_PRODUTO);
CREATE INDEX FK_COMBO_PROD_ITEM
  ON PRODUTO_COMBO_ITEM (ID_PRODUTO_COMBO);
...
CREATE TABLE EMPRESA_PRODUTO (
  ...
  ID_EMPRESA         INTEGER NOT NULL,
  QUANTIDADE_ESTOQUE DECIMAL(18, 6),
  ESTOQUE_VERIFICADO DECIMAL(18, 6),
  PRIMARY KEY (ID),
  FOREIGN KEY (ID_PRODUTO)
  REFERENCES PRODUTO (ID),
  FOREIGN KEY (ID_EMPRESA)
...
CREATE INDEX FK_PRODUTO_EMPRESA
  ON EMPRESA_PRODUTO (ID_PRODUTO);
CREATE INDEX FK_EMPRESA_PRODUTO
  ON EMPRESA_PRODUTO (ID_EMPRESA);
...
CREATE INDEX FK_DAV_CAB_DET
  ON DAV_DETALHE (ID_DAV_CABECALHO);
CREATE INDEX FK_PRODUTO_DAV_DETALHE
  ON DAV_DETALHE (ID_PRODUTO);
...
CREATE INDEX FK_ESTOQUE_REAJUSTE_CAB_DET
  ON ESTOQUE_REAJUSTE_DETALHE (ID_ESTOQUE_REAJUSTE_CABECALHO);
CREATE INDEX FK_PRODUTO_REAJUSTE
  ON ESTOQUE_REAJUSTE_DETALHE (ID_PRODUTO);
...
CREATE INDEX FK_DOC_ORI_LANC_RECEBER
  ON FIN_LANCAMENTO_RECEBER (ID_FIN_DOCUMENTO_ORIGEM);
CREATE INDEX FK_CLIENTE_LCTO_RECEBER
  ON FIN_LANCAMENTO_RECEBER (ID_CLIENTE);
...
CREATE INDEX FK_FIN_CLIENTE_GRUPO
  ON FIN_CLIENTE_GRUPO_DET (ID_FIN_CLIENTE_GRUPO_CAB);
CREATE INDEX FK_CLIENTE_FIN_CLIENTE_GRUPO
  ON FIN_CLIENTE_GRUPO_DET (ID_CLIENTE);
...
CREATE INDEX FK_COLABORADOR_RELACIONAMENTO
  ON COLABORADOR_RELACIONAMENTO (ID_COLABORADOR);
CREATE INDEX FK_TIPO_REL_COLABORADOR
  ON COLABORADOR_RELACIONAMENTO (ID_TIPO_RELACIONAMENTO);
...
CREATE INDEX FK_FORNECEDOR_COTACAO_DETALHE
  ON COMPRA_COTACAO_DETALHE (ID_COMPRA_FORNECEDOR_COTACAO);
CREATE INDEX FK_PRODUTO_COTACAO_DETALHE
  ON COMPRA_COTACAO_DETALHE (ID_PRODUTO);
...
CREATE INDEX FK_PEDIDO_COMPRA_DETALHE
  ON COMPRA_PEDIDO_DETALHE (ID_COMPRA_PEDIDO);
CREATE INDEX FK_PROD_PEDIDO_DETALHE
  ON COMPRA_PEDIDO_DETALHE (ID_PRODUTO);
...
CREATE INDEX FK_NF_CR
  ON CT_RESULTADO_NT_FINANCEIRA (ID_NATUREZA_FINANCEIRA);
CREATE INDEX CT_RESULTADO_NT_FINANCEIRA_FKIndex2
  ON CT_RESULTADO_NT_FINANCEIRA (ID_CENTRO_RESULTADO);
...
CREATE INDEX FK_TIPO_REQ_COMPRA
  ON COMPRA_REQUISICAO (ID_COMPRA_TIPO_REQUISICAO);
CREATE INDEX FK_COL_REQ_COMPRA
  ON COMPRA_REQUISICAO (ID_COLABORADOR);
...
CREATE INDEX FK_REQUISICAO_COMPRA_DETALHE
  ON COMPRA_REQUISICAO_DETALHE (ID_COMPRA_REQUISICAO);
CREATE INDEX FK_PRODUTO_REQ_COMPRA_DET
  ON COMPRA_REQUISICAO_DETALHE (ID_PRODUTO);
...
CREATE INDEX FK_PRODUTO_FORNECEDOR
  ON FORNECEDOR_PRODUTO (ID_PRODUTO);
CREATE INDEX FK_FORNECEDOR_PRODUTO
...
CREATE INDEX FK_CONTAGEM_CAB_DET
  ON INVENTARIO_CONTAGEM_DET (ID_INVENTARIO_CONTAGEM_CAB);
CREATE INDEX FK_PRODUTO_CONTAGEM_INVENTARIO
  ON INVENTARIO_CONTAGEM_DET (ID_PRODUTO);

-- ------------------------------------------------------------
-- Para controlar a movimentação e posição do produto em estoque.
-- ------------------------------------------------------------

CREATE TABLE ESTOQUE_PRODUTO_MOVIMENTACAO (
  ID                 SERIAL  NOT NULL,
  id_empresa_produto INTEGER,
  ID_PRODUTO         INTEGER NOT NULL,
  codigo_modulo      VARCHAR(3),
  DOCUMENTO          VARCHAR(50),
  data_movimento     DATE,
  QUANTIDADE         DECIMAL(18, 6),
  VALOR_CUSTO        DECIMAL(18, 6),
  SALDO_FISICO       DECIMAL(18, 6),
...
CREATE INDEX FK_LANC_REC_NAT_FIN
  ON FIN_LCTO_RECEBER_NT_FINANCEIRA (ID_FIN_LANCAMENTO_RECEBER);
CREATE INDEX FK_NAT_FIN_LANC_RECEBER
  ON FIN_LCTO_RECEBER_NT_FINANCEIRA (ID_NATUREZA_FINANCEIRA);
...
CREATE INDEX FK_LANC_PG_NAT_FIN
  ON FIN_LCTO_PAGAR_NT_FINANCEIRA (ID_FIN_LANCAMENTO_PAGAR);
CREATE INDEX FK_NAT_FIN_LANC_PAGAR
  ON FIN_LCTO_PAGAR_NT_FINANCEIRA (ID_NATUREZA_FINANCEIRA);
...
CREATE INDEX FK_PRE_VENDA_CAB_DET
  ON PRE_VENDA_DETALHE (ID_PRE_VENDA_CABECALHO);
CREATE INDEX FK_PRODUTO_PV_DETALHE
  ON PRE_VENDA_DETALHE (ID_PRODUTO);
...
CREATE INDEX FK_COMISSAO_PERFIL_OBJETIVO
  ON COMISSAO_OBJETIVO (ID_COMISSAO_PERFIL);
CREATE INDEX FK_PRODUTO_COMISSAO_OBJETIVO
  ON COMISSAO_OBJETIVO (ID_PRODUTO);
CREATE INDEX FK_TABELA_PRECO_COMISSAO
  ON COMISSAO_OBJETIVO (ID_TABELA_PRECO);
...
CREATE INDEX FK_LANCAMENTO_PARCELA_RECEBER
  ON FIN_PARCELA_RECEBER (ID_FIN_LANCAMENTO_RECEBER);
CREATE INDEX FK_STATUS_PARCELA_RECEBER
  ON FIN_PARCELA_RECEBER (ID_FIN_STATUS_PARCELA);
CREATE INDEX FK_CTA_CX_PAR_REC
  ON FIN_PARCELA_RECEBER (ID_CONTA_CAIXA);
...
CREATE INDEX FK_PARCELA_RECEB
  ON FIN_PARCELA_RECEBIMENTO (ID_FIN_PARCELA_RECEBER);
CREATE INDEX FK_TIPO_PARC_RECEBIMENTO
  ON FIN_PARCELA_RECEBIMENTO (ID_FIN_TIPO_RECEBIMENTO);
CREATE INDEX FK_CONTA_CAIXA_PARC_REC
  ON FIN_PARCELA_RECEBIMENTO (ID_CONTA_CAIXA);
CREATE INDEX FK_CHEQUE_RECEBIMENTO
  ON FIN_PARCELA_RECEBIMENTO (ID_FIN_CHEQUE_RECEBIDO);

-- ------------------------------------------------------------
-- Tabela que controla os recebimentos em cartão repassados pelas operadoras.
-- ------------------------------------------------------------

CREATE TABLE FIN_RECEBIMENTO_CARTAO (
  ID                         SERIAL  NOT NULL,
  ID_FIN_PARCELA_RECEBIMENTO INTEGER NOT NULL,
  ...
  VALOR_LIQUIDO              DECIMAL(18, 6),
  PRIMARY KEY (ID),
  FOREIGN KEY (ID_OPERADORA_CARTAO)
  REFERENCES OPERADORA_CARTAO (ID),
  FOREIGN KEY (ID_FIN_PARCELA_RECEBIMENTO)
  REFERENCES FIN_PARCELA_RECEBIMENTO (ID)
);


CREATE INDEX FK_OPERADORA_CARTAO_FIN_REC
  ON FIN_RECEBIMENTO_CARTAO (ID_OPERADORA_CARTAO);
CREATE INDEX FK_FIN_PARC_REC_CARTAO
  ON FIN_RECEBIMENTO_CARTAO (ID_FIN_PARCELA_RECEBIMENTO);
CREATE INDEX IFK_FK_OPERADORA_CARTAO_FIN_RE
  ON FIN_RECEBIMENTO_CARTAO (ID_OPERADORA_CARTAO);
CREATE INDEX IFK_FK_FIN_PARC_REC_CARTAO
  ON FIN_RECEBIMENTO_CARTAO (ID_FIN_PARCELA_RECEBIMENTO);
...
CREATE INDEX FK_PARCELA_PAGAMENTO
  ON FIN_PARCELA_PAGAMENTO (ID_FIN_PARCELA_PAGAR);
CREATE INDEX FK_CHEQUE_PARCELA
  ON FIN_PARCELA_PAGAMENTO (ID_FIN_CHEQUE_EMITIDO);
CREATE INDEX FK_TIPO_PAGAMENTO_PARCELA
  ON FIN_PARCELA_PAGAMENTO (ID_FIN_TIPO_PAGAMENTO);
CREATE INDEX FK_CONTA_CX_PARC_PGTO
  ON FIN_PARCELA_PAGAMENTO (ID_CONTA_CAIXA);
...
CREATE INDEX FK_COLABORADOR_VENDEDOR
  ON VENDEDOR (ID_COLABORADOR);
CREATE INDEX FK_COMISSAO_PERFIL_VENDEDOR
  ON VENDEDOR (ID_COMISSAO_PERFIL);
CREATE INDEX FK_REGIAO_VENDEDOR
  ON VENDEDOR (ID_REGIAO);
CREATE INDEX FK_TABELA_PRECO_VENDEDOR
  ON VENDEDOR (ID_TABELA_PRECO);
...
CREATE INDEX FK_COND_PGTO_ORC_PED_VENDA
  ON VENDA_ORCAMENTO_CABECALHO (ID_VENDA_CONDICOES_PAGAMENTO);
CREATE INDEX FK_TRANSP_ORC_PED_VENDA
  ON VENDA_ORCAMENTO_CABECALHO (ID_TRANSPORTADORA);
CREATE INDEX FK_CLIENTE_ORC_PED_VENDA
  ON VENDA_ORCAMENTO_CABECALHO (ID_CLIENTE);
CREATE INDEX FK_VENDEDOR_ORC_PED_VENDA
  ON VENDA_ORCAMENTO_CABECALHO (ID_VENDEDOR);
CREATE INDEX FK_EMPRESA_ORCAMENTO_CAB
  ON VENDA_ORCAMENTO_CABECALHO (ID_EMPRESA);
...
CREATE INDEX FK_ESTOQUE_COR_GRADE
  ON ESTOQUE_GRADE (ID_ESTOQUE_COR);
CREATE INDEX FK_ESTOQUE_TAMANHO_GRADE
  ON ESTOQUE_GRADE (ID_ESTOQUE_TAMANHO);
CREATE INDEX FK_PRODUTO_ESTOQUE_GRADE
  ON ESTOQUE_GRADE (ID_PRODUTO);
CREATE INDEX FK_ESTOQUE_SABOR
  ON ESTOQUE_GRADE (ID_ESTOQUE_SABOR);
CREATE INDEX FK_ESTOQUE_MARCA
  ON ESTOQUE_GRADE (ID_ESTOQUE_MARCA);
CREATE INDEX FK_EMPRESA_PRODUTO_GRADE
  ON ESTOQUE_GRADE (ID_EMPRESA_PRODUTO);

-- ------------------------------------------------------------
-- Tabela que armazena o controle de caixa
-- ------------------------------------------------------------
...
(
  ...
  id_operador_caixa     INTEGER NOT NULL,
  id_turno_caixa        INTEGER NOT NULL,
  id_empresa            INTEGER NOT NULL,
  id_gerente_supervisor INTEGER NOT NULL,
  data_abertura         DATE,
  hora_abertura         CHARACTER VARYING(8),
  data_fechamento       DATE,
  hora_fechamento       CHARACTER VARYING(8),
  ...
  status_movimento      CHARACTER(1),
  PRIMARY KEY (id),
  FOREIGN KEY (id_empresa) REFERENCES empresa (id),
  FOREIGN KEY (id_caixa) REFERENCES caixa (id),
  FOREIGN KEY (id_operador_caixa) REFERENCES operador_caixa (id),
...
CREATE INDEX FK_ORCAMENTO_VENDA
  ON VENDA_CABECALHO (ID_VENDA_ORCAMENTO_CABECALHO);
CREATE INDEX FK_VENDA_CAB_CONDICOES
  ON VENDA_CABECALHO (ID_VENDA_CONDICOES_PAGAMENTO);
CREATE INDEX FK_CLIENTE_VENDA_CAB
  ON VENDA_CABECALHO (ID_CLIENTE);
CREATE INDEX FK_VENDEDOR_VENDA_CAB
  ON VENDA_CABECALHO (ID_VENDEDOR);
CREATE INDEX FK_TIPO_NF_VENDA_CAB
  ON VENDA_CABECALHO (ID_TIPO_NOTA_FISCAL);
CREATE INDEX FK_TRANSP_VENDA_CAB
  ON VENDA_CABECALHO (ID_TRANSPORTADORA);
CREATE INDEX FK_VENDA_ROMANEIO
...
CREATE TABLE VENDA_ENTREGA_FUTURA (
  ID                 SERIAL  NOT NULL,
  ID_VENDA_CABECALHO INTEGER NOT NULL,
  TOTAL_PARCIAL      CHAR(1),
  DATA_ENTREGA       DATE,
  OBSERVACAO         TEXT,
  PRIMARY KEY (ID),
...
CREATE INDEX FK_VENDA_ENTREGA_FUTURA
  ON VENDA_ENTREGA_FUTURA (ID_VENDA_CABECALHO);


CREATE INDEX IFK_FK_VENDA_ENTREGA_FUTURA
  ON VENDA_ENTREGA_FUTURA (ID_VENDA_CABECALHO);
...
CREATE INDEX FK_CTE_DOC_ANTERIOR_ID
  ON CTE_DOCUMENTO_ANTERIOR_ID (ID_CTE_DOCUMENTO_ANTERIOR);
...
CREATE INDEX FK_USUARIO_AUDITORIA
...
CREATE INDEX DAV_DETALHE_ALTERACAO_FKIndex1
  ON DAV_DETALHE_ALTERACAO (ID_DAV_DETALHE);
...
CREATE INDEX FK_VENDA_CABECALHO_FRETE
  ON VENDA_FRETE (ID_VENDA_CABECALHO);
CREATE INDEX FK_TRANSPORTADORA_FRETE_VENDA
  ON VENDA_FRETE (ID_TRANSPORTADORA);
...
CREATE INDEX FK_VENDA_ORCAMENTO_CAB_DET
  ON VENDA_ORCAMENTO_DETALHE (ID_VENDA_ORCAMENTO_CABECALHO);
CREATE INDEX FK_PRODUTO_ORC_VENDA_DET
  ON VENDA_ORCAMENTO_DETALHE (ID_PRODUTO);
...
CREATE INDEX FK_VENDA_CAB_DET
  ON VENDA_DETALHE (ID_VENDA_CABECALHO);
CREATE INDEX FK_PRODUTO_VENDA_DET
  ON VENDA_DETALHE (ID_PRODUTO);
...
CREATE INDEX FK_VENDA_COMISSAO
  ON VENDA_COMISSAO (ID_VENDA_CABECALHO);
CREATE INDEX FK_VENDEDOR_COMISSAO
  ON VENDA_COMISSAO (ID_VENDEDOR);
...
CREATE TABLE VENDA_DEVOLUCAO (
  ...
  CREDITO_UTILIZADO   CHAR(1),
  ID_VENDA_UTILIZACAO INTEGER,
  PRIMARY KEY (ID),
  FOREIGN KEY (ID_VENDA_CABECALHO)
  REFERENCES VENDA_CABECALHO (ID)
);
...
CREATE INDEX FK_VENDA_DEVOLUCAO
  ON VENDA_DEVOLUCAO (ID_VENDA_CABECALHO);
CREATE INDEX IFK_FK_VENDA_DEVOLUCAO
  ON VENDA_DEVOLUCAO (ID_VENDA_CABECALHO);
...
CREATE INDEX FK_COT_PEDIDO_DET
  ON COMPRA_COTACAO_PEDIDO_DETALHE (ID_COMPRA_COTACAO_DETALHE);
CREATE INDEX FK_PEDIDO_COTACAO_DETALHE
  ON COMPRA_COTACAO_PEDIDO_DETALHE (ID_COMPRA_PEDIDO);
...
CREATE INDEX FK_REQUISICAO_COTACAO_DETALHE
  ON COMPRA_REQ_COTACAO_DETALHE (ID_COMPRA_REQUISICAO_DETALHE);
...
CREATE INDEX FK_CLIENTE_NFE
  ON NFE_CABECALHO (ID_CLIENTE);
CREATE INDEX FK_FORNECEDOR_NFE
  ON NFE_CABECALHO (ID_FORNECEDOR);
CREATE INDEX FK_EMPRESA_NFE
  ON NFE_CABECALHO (ID_EMPRESA);
CREATE INDEX FK_VENDA_CAB_NFE
  ON NFE_CABECALHO (ID_VENDA_CABECALHO);
CREATE INDEX FK_TRIBUT_NFE
...
CREATE INDEX FK_NFE_RURAL_REFERENCIADO
  ON NFE_PROD_RURAL_REFERENCIADA (ID_NFE_CABECALHO);
...
CREATE INDEX NFE_CAB_PROC_REF
  ON NFE_PROCESSO_REFERENCIADO (ID_NFE_CABECALHO);
...
CREATE INDEX FK_NFE_REFERENCIADA
  ON NFE_REFERENCIADA (ID_NFE_CABECALHO);
...
CREATE INDEX FK_NFE_CAB_FATURA
  ON NFE_FATURA (ID_NFE_CABECALHO);
...
CREATE INDEX FK_NFE_NF_REFERENCIADA
  ON NFE_NF_REFERENCIADA (ID_NFE_CABECALHO);
...
CREATE INDEX FK_NFE_LOCAL_RETIRADA
  ON NFE_LOCAL_RETIRADA (ID_NFE_CABECALHO);
...
CREATE INDEX FK_NFE_LOCAL_ENTREGA
  ON NFE_LOCAL_ENTREGA (ID_NFE_CABECALHO);
...
CREATE INDEX FK_NFE_CTE_REFERENCIADO
  ON NFE_CTE_REFERENCIADO (ID_NFE_CABECALHO);
...
CREATE INDEX FK_NFE_DESTINATARIO
  ON NFE_DESTINATARIO (ID_NFE_CABECALHO);
...
CREATE INDEX FK_NFE_CF_REFERENCIADO
  ON NFE_CUPOM_FISCAL_REFERENCIADO (ID_NFE_CABECALHO);
...
CREATE INDEX FK_NFE_CAB_CANA
  ON NFE_CANA (ID_NFE_CABECALHO);
...
CREATE INDEX FK_ACESSO_XML
  ON NFE_ACESSO_XML (ID_NFE_CABECALHO);
...
CREATE INDEX FK_NFE_EMITENTE
  ON NFE_EMITENTE (ID_NFE_CABECALHO);
...
CREATE INDEX FK_NFE_CAB_DUPLICATA
  ON NFE_DUPLICATA (ID_NFE_CABECALHO);
...
CREATE INDEX FK_PRODUTO_LOTE
  ON PRODUTO_LOTE (ID_PRODUTO);
CREATE INDEX FK_NFE_CAB_PROD_LOTE
  ON PRODUTO_LOTE (ID_NFE_CABECALHO);
...
CREATE INDEX FK_NFE_CAB_DET
  ON NFE_DETALHE (ID_NFE_CABECALHO);
CREATE INDEX FK_PRODUTO_NFE
  ON NFE_DETALHE (ID_PRODUTO);
...
CREATE INDEX FK_NFE_TRANSPORTE
  ON NFE_TRANSPORTE (ID_NFE_CABECALHO);
CREATE INDEX FK_TRANSPORTADORA_NFE_TRANSP
  ON NFE_TRANSPORTE (ID_TRANSPORTADORA);
...
CREATE INDEX FK_NFE_TRANSP_REBOQUE
  ON NFE_TRANSPORTE_REBOQUE (ID_NFE_TRANSPORTE);
...
CREATE INDEX FK_TRANSP_VOLUME
  ON NFE_TRANSPORTE_VOLUME (ID_NFE_TRANSPORTE);
...
CREATE INDEX FK_NFE_EXPORTACAO
  ON NFE_EXPORTACAO (ID_NFE_DETALHE);
...
CREATE INDEX FK_NFE_CANA_FORNECIMENTO
  ON NFE_CANA_FORNECIMENTO_DIARIO (ID_NFE_CANA);
...
CREATE INDEX FK_NFE_DET_DEC_IMPORTACAO
  ON NFE_DECLARACAO_IMPORTACAO (ID_NFE_DETALHE);
...
CREATE INDEX FK_NFE_CANA_DEDUCOES
  ON NFE_CANA_DEDUCOES_SAFRA (ID_NFE_CANA);
...
CREATE INDEX FK_NFE_DET_ESP_MEDICAMENTO
  ON NFE_DET_ESPECIFICO_MEDICAMENTO (ID_NFE_DETALHE);
...
CREATE INDEX FK_NFE_DET_ESP_COMBUSTIVEL
  ON NFE_DET_ESPECIFICO_COMBUSTIVEL (ID_NFE_DETALHE);
...
CREATE INDEX FK_NFE_DET_ESP_ARMAMENTO
  ON NFE_DET_ESPECIFICO_ARMAMENTO (ID_NFE_DETALHE);
...
CREATE INDEX FK_NFE_DET_ESPECIFICO_VEICULO
  ON NFE_DET_ESPECIFICO_VEICULO (ID_NFE_DETALHE);
...
CREATE INDEX FK_NFE_DETALHE_RASTRO
  ON NFE_DETALHE_RASTRO (ID_NFE_DETALHE);
...
CREATE INDEX FK_NFE_DET_II
  ON NFE_DETALHE_IMPOSTO_II (ID_NFE_DETALHE);
...
CREATE INDEX FK_NFE_DET_ICMS
  ON NFE_DETALHE_IMPOSTO_ICMS (ID_NFE_DETALHE);
...
CREATE INDEX FK_NFE_DET_COFINS
  ON NFE_DETALHE_IMPOSTO_COFINS (ID_NFE_DETALHE);
...
CREATE INDEX FK_NFE_DET_PIS
  ON NFE_DETALHE_IMPOSTO_PIS (ID_NFE_DETALHE);
...
CREATE INDEX FK_NFE_DET_ISSQN
  ON NFE_DETALHE_IMPOSTO_ISSQN (ID_NFE_DETALHE);

-- ------------------------------------------------------------
-- Essa tabela não tem seus campos definidos no esquema do XML do manual da NF-e. Foi criada para realizar o controle da retenção de impostos quando da emissão de uma nota fiscal.
-- ------------------------------------------------------------

CREATE TABLE NFE_RETENCAO_IMPOSTO (
  ID               SERIAL  NOT NULL,
  ID_NFE_CABECALHO INTEGER NOT NULL,
  DISPENSADO       CHAR(1),
  ALIQUOTA_PIS     DECIMAL(18, 6),
  RETENCAO_PIS     DECIMAL(18, 6),
  ALIQUOTA_COFINS  DECIMAL(18, 6),
  RETENCAO_COFINS  DECIMAL(18, 6),
  ALIQUOTA_IRRF    DECIMAL(18, 6),
  RETENCAO_IRRF    DECIMAL(18, 6),
  ALIQUOTA_CSLL    DECIMAL(18, 6),
  RETENCAO_CSLL    DECIMAL(18, 6),
  ALIQUOTA_INSS    DECIMAL(18, 6),
  RETENCAO_INSS    DECIMAL(18, 6),
  ALIQUOTA_ISS     DECIMAL(18, 6),
  RETENCAO_ISS     DECIMAL(18, 6),
  PRIMARY KEY (ID),
  FOREIGN KEY (ID_NFE_CABECALHO)
  REFERENCES NFE_CABECALHO (ID)
);
...
CREATE INDEX FK_NFE_RETENCAO
  ON NFE_RETENCAO_IMPOSTO (ID_NFE_CABECALHO);

CREATE INDEX IFK_FK_NFE_RETENCAO
  ON NFE_RETENCAO_IMPOSTO (ID_NFE_CABECALHO);
...
CREATE INDEX FK_NFE_DET_IPI
  ON NFE_DETALHE_IMPOSTO_IPI (ID_NFE_DETALHE);
...
CREATE INDEX FK_NFE_IMPORTACAO_DETALHE
  ON NFE_IMPORTACAO_DETALHE (ID_NFE_DECLARACAO_IMPORTACAO);
...
CREATE INDEX FK_NFE_TRANSP_VOL_LACRE
  ON NFE_TRANSPORTE_VOLUME_LACRE (ID_NFE_TRANSPORTE_VOLUME);
...
CREATE INDEX FK_NFE_OS_ABERTURA
  ON OS_ABERTURA (ID_NFE_CABECALHO);
...
(
  ...
  CONSTRAINT fiscal_apuracao_icms_id_empresa_fkey FOREIGN KEY (id_empresa) REFERENCES empresa (id)
);
...
(
  ...
  CONSTRAINT agenda_compromisso_id_colaborador_fkey FOREIGN KEY (id_colaborador) REFERENCES colaborador (id)
);
...
(
  ...
  CONSTRAINT agenda_compromisso_convidado_id_colaborador_fkey FOREIGN KEY (id_colaborador) REFERENCES colaborador (id)
);
...
COMMENT ON COLUMN agenda_notificacao.tipo IS '0=e-mail | 1=mensagem na tela do computador';


CREATE TABLE reuniao_sala
(
  id     SERIAL NOT NULL,
  predio CHARACTER VARYING(100),
  andar  CHARACTER VARYING(10),
  numero CHARACTER VARYING(10),
  CONSTRAINT reuniao_sala_pkey PRIMARY KEY (id)
);


CREATE TABLE reuniao_sala_evento
(
  id                    SERIAL  NOT NULL,
  id_agenda_compromisso INTEGER NOT NULL,
  id_reuniao_sala       INTEGER NOT NULL,
  data_reserva          DATE,
  CONSTRAINT reuniao_sala_evento_pkey PRIMARY KEY (id),
  CONSTRAINT reuniao_sala_evento_id_agenda_compromisso_fkey FOREIGN KEY (id_agenda_compromisso)
  REFERENCES agenda_compromisso (id) MATCH SIMPLE
  ON UPDATE NO ACTION ON DELETE NO ACTION,
  CONSTRAINT reuniao_sala_evento_id_reuniao_sala_fkey FOREIGN KEY (id_reuniao_sala)
  REFERENCES reuniao_sala (id) MATCH SIMPLE
  ON UPDATE NO ACTION ON DELETE NO ACTION
);
...
CREATE INDEX FK_MDFE_MUNICIPIO_DESCARREGAMENTO
...
CREATE INDEX FK_MDFE_RODOVIARIO_VEICULO
  ON MDFE_RODOVIARIO_VEICULO (ID_MDFE_RODOVIARIO);
CREATE INDEX IFK_FK_MDFE_RODOVIARIO_VEICULO
  ON MDFE_RODOVIARIO_VEICULO (ID_MDFE_RODOVIARIO);
...
CREATE INDEX FK_MDFE_RODOVIARIO_PEDAGIO
...
CREATE INDEX FK_MDFE_RODOVIARIO_MOTORISTA
...
CREATE INDEX FK_MDFE_INFORMACAO_NFE
...
CREATE INDEX FK_MDFE_INFORMACAO_CTE
...
CREATE INDEX FK_MDFE_MUNICIPIO_CARREGAMENTO
...
CREATE INDEX FK_MDFE_INFORMACAO_SEGURO
